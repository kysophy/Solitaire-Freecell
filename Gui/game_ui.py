...
from Solver import solve_astar as run_astar, solve_ucs as run_ucs, apply_solution
...
class FreeCell:
    ...
    def __init__(self, root):
        ...
        self._ucs_base_state = None
        self._last_preview_len = -1 
    ...
    def _on_solve_ucs_done(self, actions):
        self._solving = False
        self.ucs_btn.config(state="normal", text="UCS")

        if actions is None:
            self._status_msg = "UCS could not solve — try New Game"
            self._status_until = time.time() + 3
            return

        from Solver.ucs import LAST_RUN_STATS
        if LAST_RUN_STATS:
            print("UCS stats:", LAST_RUN_STATS)
    ...
    def _restore_board_from_snapshot(self):
        if self._ucs_base_state is None:
            return
        tab, fc, fd = self._ucs_base_state
        self.tableau = copy.deepcopy(tab)
        self.freecells = list(fc)
        self.foundations = copy.deepcopy(fd)

    def _start_preview_animation(self, actions):
        self._preview_ticket += 1
        ticket = self._preview_ticket

        self._restore_board_from_snapshot()
        ...
        self._preview_next(ticket)

    def _preview_next(self, ticket):
        if ticket != self._preview_ticket:
            return

        if not self._preview_actions:
            return

        action = self._preview_actions.pop(0)
        apply_solution([action], self.tableau, self.freecells, self.foundations)
        ...
        self.root.after(120, lambda: self._preview_next(ticket))
    
    def _on_ucs_progress(self, payload):
        if self._current_replay_gen != self._solve_generation:
            return

        actions = payload["actions"]

        if len(actions) == self._last_preview_len:
            return

        self._last_preview_len = len(actions)

        self._status_msg = (
            f'UCS searching... t={payload["elapsed"]:.1f}s, '
            f'g={payload["current_cost"]}, '
            f'fd={payload["foundation_cards"]}, '
            f'exp={payload["expanded_nodes"]}'
        )
        self._status_until = time.time() + 1

        if actions:
            self._start_preview_animation(actions)

    def _on_ucs_done(self, result):
        self._solving = False
        self.ucs_btn.config(state="normal", text="UCS")

        if result is None:
            self._status_msg = "UCS failed"
            self._status_until = time.time() + 3
            return
        ...
        if result["success"]:
            self._status_msg = "UCS solved"
        else:
            self._status_msg = f'UCS stopped: {result["reason"]}'
    ...
    def solve_ucs(self):
        ...
        self._solve_generation += 1
        self._current_replay_gen = self._solve_generation
        self.ucs_btn.config(state="disabled", text="Searching...")

        tab_snap = copy.deepcopy(self.tableau)
        fc_snap = list(self.freecells)
        fd_snap = copy.deepcopy(self.foundations)

        self._ucs_base_state = (
            copy.deepcopy(tab_snap),
            list(fc_snap),
            copy.deepcopy(fd_snap),
        )

        def _progress(payload):
            self.root.after(0, lambda p=payload: self._on_ucs_progress(p))
        ...
        def _run():
            ...
            self.root.after(0, lambda: self._on_ucs_done(result))
