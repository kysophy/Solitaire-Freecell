...
#change the live board state to tuple
def _to_tuple_state(tableau, freecells, foundations):
    tab = tuple(
        tuple((c.rank, c.suit) for c in col)
        for col in tableau
    )
    fc = tuple(
        (c.rank, c.suit) if c is not None else None
        for c in freecells
    )
    fd = tuple(
        tuple((c.rank, c.suit) for c in pile)
        for pile in foundations
    )
    return tab, fc, fd

def _encode(tab, fc, fd):
    # Each column separated by "/" so empty columns are distinct
    tab_str = "/".join(
        "|".join(f"{r}{s[0]}" for r, s in col) if col else "-"
        for col in tab
    )
    fc_str = ",".join(
        f"{c[0]}{c[1][0]}" if c else "." for c in fc
    )
    # Each foundation pile separated and uses suit initial + count
    fd_str = "/".join(
...
def _is_safe_to_foundation(card, fd):
    ...
    needed = r - 1   # the rank that must already be home
 
    # Build a quick suit->top_rank lookup from the current foundation
    fd_top = {}
    for pile in fd:
        if pile:
            top_r, top_s = pile[-1]
    ...
    # Both opposite-color suits must have at least `needed` on foundation
...
def _remove_t(tab, fc, fd, src):
    ...
    if kind == "freecell":
        new_fc = fc[:idx] + (None,) + fc[idx + 1:]
        return tab, new_fc, fd
 
    if kind == "tableau":
        new_col = tab[idx][:-1]
        new_tab = tab[:idx] + (new_col,) + tab[idx + 1:]
        return new_tab, fc, fd
 
    if kind == "foundation":
        new_pile = fd[idx][:-1]
        new_fd   = fd[:idx] + (new_pile,) + fd[idx + 1:]
        return tab, fc, new_fd
...
def solve(tableau, freecells, foundations, max_states=2_000_000, timeout_sec=30):
    ...
    while heap:
        ...
        if all(len(pile) == 13 for pile in fd):
            ...
            # Prepend the initial auto moves to the full path
            return init_auto + _reconstruct_path(came_from, enc)

        # Expand successors
        for primary, new_tab, new_fc, new_fd in _successors(tab, fc, fd):
            # Run auto_foundation on the successor state
            # and store its auto moves WITH that successor in came_from
            new_tab, new_fc, new_fd, auto = _auto_foundation(
                new_tab, new_fc, new_fd
            )
            new_g   = g + 1
            ...
            if new_g < visited.get(new_enc, float('inf')):
                ...
                # Store auto moves that belong TO this successor state
...
def _move_tableau_to_freecell(tableau, freecells, col_index, slot_index, card):
    tableau[col_index].pop()          # remove from source column
