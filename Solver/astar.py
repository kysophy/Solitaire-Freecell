...
def solve(tableau, freecells, foundations, max_states=2_000_000, timeout_sec=120):
    ...
    while heap:
        ...
        if expansions % 50000 == 0:
            print(f"\r[A*] Expanded: {expansions:,} | g={g} | f={f} | heap={len(heap):,}", end="", flush=True)
        ...
        for primary, new_tab, new_fc, new_fd in _successors(tab, fc, fd, prev_move):
            ...
            if new_g < visited.get(new_enc, float('inf')):
                ...
                new_f = new_g + h
                tie_counter += 1
                heapq.heappush(heap, (new_f, new_g, tie_counter, new_tab, new_fc, new_fd))

    elapsed = time.time() - start_time
    current, peak = tracemalloc.get_traced_memory()
    print("\n\n--- A* Results ---")
    print("Status: HEAP EMPTY (No Solution)")
    print(f"Search Time: {elapsed:.3f} seconds")
    print(f"Memory Usage (Current): {current / 10**6:.2f} MB")
    print(f"Memory Usage (Peak): {peak / 10**6:.2f} MB")
    print(f"Expanded Nodes: {expansions}")
