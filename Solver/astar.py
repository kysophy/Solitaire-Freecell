...
def solve(tableau, freecells, foundations, max_states=2_000_000, timeout_sec=30, cancel_event=None):
    import time
    import tracemalloc
    ...
    while heap:
        ...
        if (cancel_event and cancel_event.is_set()) or time.time() > deadline:
            ...
            print(f"{'CANCELLED' if cancel_event and cancel_event.is_set() else 'TIMEOUT'} at {expansions} expansions")
        ...
        if all(len(pile) == 13 for pile in fd):
            elapsed = time.time() - start_time
            solution = init_auto + _reconstruct_path(came_from, enc)
            print(f"Time: {elapsed:.3f} seconds")
